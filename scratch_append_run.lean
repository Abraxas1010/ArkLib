/- Step 2 of the append program: Prover.append_run, attacked at the 1-round ++
   1-round instance (the BCS commit++reveal shape). THE BREAKTHROUGH: their
   tactic-mode `by_cases` field definitions block defeq with propositional
   `Eq.mpr` casts even at literal indices — but the casts fall to
     simp only [Prover.append, eq_mpr_eq_cast, eq_mp_eq_cast]
     rw [dif_pos/dif_neg ...]
     refine eq_of_heq ((cast_heq _ _).trans ?_)
     congr 1
   (cast-on-defeq killed by heq-transitivity + congruence). Field equation
   lemmas below; assembly plan at the bottom. -/
...
theorem bridge_c {α : Type} (X : OracleComp oSpec α) :
  ...
  exact @QueryImpl.simulateQ_liftM_eq_of_query _ _ oSpec _ _ _ _ _
    ...
    (fun t => by
      ...
      simp [QueryImpl.addLift_def, OracleComp.liftComp_query, simulateQ_query,
        OracleSpec.query_def, simulateQ_spec_query, OracleSpec.SubSpec.liftQuery,
        OracleQuery.liftM_add_left_def, QueryImpl.add_apply_inl]
...
/-! ## The composition theorem (from scratch_append_completeness.lean) + THE CROWN -/
...
/-- **THE CROWN — 1-message append perfect completeness, unconditional**: for
